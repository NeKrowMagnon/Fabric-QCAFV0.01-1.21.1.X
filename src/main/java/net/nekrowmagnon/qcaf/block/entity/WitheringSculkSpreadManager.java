...
import java.util.*;
/*
public class WitheringSculkSpreadManager {
    ...
    private List<WitheringSculkSpreadManager.Cursor> cursors = new ArrayList<>();
    private static final Logger LOGGER = LogUtils.getLogger();


    public WitheringSculkSpreadManager(boolean worldGen, TagKey<Block> replaceableTag, int extraBlockChance, int maxDistance, int spreadChance, int decayChance) {
    ...
    public static WitheringSculkSpreadManager create() {
        return new WitheringSculkSpreadManager(false, BlockTags.SCULK_REPLACEABLE, 10, 4, 10, 5);
    }

    public static WitheringSculkSpreadManager createWorldGen() {
        return new WitheringSculkSpreadManager(true, BlockTags.SCULK_REPLACEABLE_WORLD_GEN, 50, 1, 5, 10);
    }
    ...
    public void readNbt(NbtCompound nbt) {
        if (nbt.contains("cursors", NbtElement.LIST_TYPE)) {
            this.cursors.clear();
            List<WitheringSculkSpreadManager.Cursor> list = (List<WitheringSculkSpreadManager.Cursor>)WitheringSculkSpreadManager.Cursor.CODEC
                    .listOf()
            ...
            for (int j = 0; j < i; j++) {
                this.addCursor((WitheringSculkSpreadManager.Cursor)list.get(j));
            }
        }
    }

    public void writeNbt(NbtCompound nbt) {
        WitheringSculkSpreadManager.Cursor.CODEC
                .listOf()
    ...
    public void spread(BlockPos pos, int charge) {
        while (charge > 0) {
            ...
            this.addCursor(new WitheringSculkSpreadManager.Cursor(pos, i));
            charge -= i;
        }
    }

    private void addCursor(WitheringSculkSpreadManager.Cursor cursor) {
    ...
    public void tick(WorldAccess world, BlockPos pos, Random random, boolean shouldConvertToBlock) {
        if (!this.cursors.isEmpty()) {
            List<WitheringSculkSpreadManager.Cursor> list = new ArrayList<>();
            Map<BlockPos, WitheringSculkSpreadManager.Cursor> map = new HashMap<>();
            Object2IntMap<BlockPos> object2IntMap = new Object2IntOpenHashMap<>();

            for (WitheringSculkSpreadManager.Cursor cursor : this.cursors) {
                ...
                if (cursor.charge <= 0) {
                    ...
                    WitheringSculkSpreadManager.Cursor cursor2 = (WitheringSculkSpreadManager.Cursor)map.get(blockPos);
                    if (cursor2 == null) {
            ...
            for (Object2IntMap.Entry<BlockPos> entry : object2IntMap.object2IntEntrySet()) {
                ...
                WitheringSculkSpreadManager.Cursor cursor3 = (WitheringSculkSpreadManager.Cursor)map.get(blockPos);
    ...
    public static class Cursor {
        ...
        public static final Codec<WitheringSculkSpreadManager.Cursor> CODEC = RecordCodecBuilder.create(
                instance -> instance.group(
                                BlockPos.CODEC.fieldOf("pos").forGetter(WitheringSculkSpreadManager.Cursor::getPos),
                                Codec.intRange(0, 1000).fieldOf("charge").orElse(0).forGetter(WitheringSculkSpreadManager.Cursor::getCharge),
                                Codec.intRange(0, 1).fieldOf("decay_delay").orElse(1).forGetter(WitheringSculkSpreadManager.Cursor::getDecay),
                        ...
                        .apply(instance, WitheringSculkSpreadManager.Cursor::new)
        );
        ...
        public void spread(WorldAccess world, BlockPos pos, Random random, WitheringSculkSpreadManager spreadManager, boolean shouldConvertToBlock) {
        ...
        void merge(WitheringSculkSpreadManager.Cursor cursor) {
